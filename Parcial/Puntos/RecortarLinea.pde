...
class RecortarLinea {  
  ...
  RecortarLinea(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, ArrayList<Punto> _puntos) {
  ...
  void print() {    
    for (Punto l : puntos) {
      l.print(color(255, 0, 0));
    }
  }
  void cut() {    
    for (Punto p : puntos) {
      //l.print(color(0,0,0));
      cohenSutherlandClip(p.x1, p.y1, p.x2, p.y2);
    }
  }
